        ...
        const struct wined3d_vk_info *vk_info, VkPhysicalDevice vk_physical_device)
{
    ...
    if (vk_format == VK_FORMAT_X8_D24_UNORM_PACK32)
    {
        if (~properties.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)
        {
            /* Metal has no 24-bit depth formats; DirectDraw-era games ask for
             * a 24-bit z-buffer before trying anything else. Both formats are
             * four bytes per pixel. */
            WARN("Mapping VK_FORMAT_X8_D24_UNORM_PACK32 to VK_FORMAT_D32_SFLOAT.\n");

            format->vk_format = vk_format = VK_FORMAT_D32_SFLOAT;
            VK_CALL(vkGetPhysicalDeviceFormatProperties(vk_physical_device, vk_format, &properties));
        }
    }

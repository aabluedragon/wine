...
- **Interpreter opt: `-O1` is FASTEST** for this switch-dispatch interpreter —
  O1 ~95 > O2 ~91 > O3 ~84 > Os ~77 Minsn/s.  `-O3` over-inlines the giant
  opcode switch and *regresses*.  (The old "-O3 would help" note was wrong.)
...
- The profile is *flat* (bread-and-butter movs/cmps/movzx/jcc) — no single hot
  handler; cost is per-instruction dispatch.  Real per-frame work is ~10-13M
  guest insns at 1024×768, so resolution is the main lever short of a
  decoded-block JIT (blocked by Build's self-modifying texture mappers).

...
Emscripten link changed from `-O2` to `-O3`. The candidate URL was
`http://localhost:9450/?WASM_TPUT=1&WW_ARGS=%2Fv1,%2Fl1&build=link3-candidate-20260906`;
its JS/WASM/data hashes were
`d778de37207f1eb4e7bdcd3d54d0a81bd0ab36bbac2032cc815eff749baf4d16`,
`d48a78f031bda3f975abfada136bb4294f34e88ff0f10c8d0911ba89a1a5adcd`, and
`b6e7c288b2cc5f9e5a83a153561d4d385f8eb073e538258ac7ebf65d947e4b63`.
The matched candidate/control late-window means were 81.4 and 54.6 FPS.
Both runs rendered changing non-black 640x400 frames, reported `input: ready`,
reached E1L1, accepted Enter/W, and had no `RuntimeError`, `JITBAD`,
`JITBADEIP`, or `FATAL`.

The canonical bundle was rebuilt and tested at
`http://localhost:8799/?WASM_TPUT=1&WW_ARGS=%2Fv1,%2Fl1&build=fp-callguard-o3-link3-published-20260906`.
It returned HTTP 200 and passed the same gate; its hashes are JS
`ee344b3c9721f75425a54ed625df657430bcb85a9eb19c3c17485acfd7c3733d`, WASM
`d48a78f031bda3f975abfada136bb4294f34e88ff0f10c8d0911ba89a1a5adcd`, data
`b6e7c288b2cc5f9e5a83a153561d4d385f8eb073e538258ac7ebf65d947e4b63`, index
`455e20ff86b48a6c3e880dd5558bc54c2f749845b2fee6ee7fa343407bd9bcc6`, and
audio worklet `a294aaa599e2505e4069dbdb67de5ace0debeb5ac4ef72a721107ec74f2b1519`.
Preserved untracked build/cache/platform artifacts remain in the dirty Wine
...
Hypothesis: link-time optimization is complementary to the interpreter `-O3`
change and improves the hot generated/helper code without changing guest
semantics. The next target remains the verified removal of the residual
interpreter work, not broader unsafe AOT roots.
...
browser renderer. The 130-block candidate added the exact SSE helper at
`0x0061663a--0x0061671b`; it rendered non-black E1L1 gameplay, accepted Enter/W,
and reported no fatal/JIT errors, but the matched `WASM_NO_FP_HOT=1` rollback
had equivalent late samples. A second candidate translated the whole
`0x00610000--0x0061ffff` renderer band (3,800 blocks); it also rendered and
accepted input, but did not improve FPS and substantially increased the AOT
table. These candidates were not promoted.

Decision: restore the exact verified 129-block floating-point table and rebuild
the browser bundle. Final control URL:
`http://localhost:8799/?WASM_TPUT=1&WW_ARGS=%2Fv1,%2Fl1&build=renderer-fp-published-39a455c3`.
The fresh control run reached E1L1, produced a real non-black gameplay frame,
accepted `wasm_input: SDL key down/up vk=0x57`, and had no `FATAL`,
`RuntimeError`, or `JITBAD`; late samples reached approximately 100--111 FPS
in that host run. Artifact hashes match the published bundle: JS
`ec9fc0864c8de9f8242b84a84d2f927c7d3b4778ebfd001ae754afc68ee1a1f3`, WASM
`97648ce8f79c17523089b50c65427e9dcaa17a4b8f43406ac1337d6fc8e0940d`, data
`b6e7c288b2cc5f9e5a83a153561d4d385f8eb073e538258ac7ebf65d947e4b63`, index
`455e20ff86b48a6c3e880dd5558bc54c2f749845b2fee6ee7fa343407bd9bcc6`, and worker
...
## 2026-09-04 16:23 IDT: sequential SSE hook rejected; verified browser build restored

Observation: the opt-in `WASM_XFORM=1` native replacement for the 68-instruction
polymost SSE matrix helper was corrected for the third/fourth output stores and
tested at `http://localhost:8807/?WASM_TPUT=1&WASM_XFORM=1&WW_ARGS=%2Fv1,%2Fl1&build=seq-xform-20260904`.
It registered successfully and reached OpenGL setup, but the browser run ended
with `FATAL worker exception: table index is out of bounds` before the first
frame. The candidate was removed and was not published.

Decision: retain the verified O2 build and 129-block floating-point table. The
canonical URL is
`http://localhost:8799/?WASM_TPUT=1&WW_ARGS=%2Fv1,%2Fl1&build=renderer-fp-published-39a455c3`.
A fresh 25-second end-to-end run reached `E1L1: HOLLYWOOD HOLOCAUST`, rendered a
non-black 640x400 gameplay frame, accepted Enter and held W input
(`wasm_input: SDL key down/up vk=0x57`), and had no `FATAL`, `RuntimeError`, or
`JITBAD`. Late samples were approximately 90--139 FPS on this host, with one
sample at 138.6 FPS. Artifact hashes are JS
`ec9fc0864c8de9f8242b84a84d2f927c7d3b4778ebfd001ae754afc68ee1a1f3`, WASM
`97648ce8f79c17523089b50c65427e9dcaa17a4b8f43406ac1337d6fc8e0940d`, data
`b6e7c288b2cc5f9e5a83a153561d4d385f8eb073e538258ac7ebf65d947e4b63`, index
`455e20ff86b48a6c3e880dd5558bc54c2f749845b2fee6ee7fa343407bd9bcc6`, and worker
...
`0x0061663a--0x0061671b` SSE block built as `floating-point hot JIT 130
translated blocks` and passed the full browser gate: E1L1, non-black 640x400
gameplay, Enter/W input, audio, and no `FATAL`, `RuntimeError`, or `JITBAD`.
An initial 25-second run showed about 127--145 FPS late, but a 30-second
repeat showed about 102--119 FPS; the matched same-artifact rollback ranged
about 100--118 FPS under the same host period. Startup also varied from 11.8
to 13.6s. This is not reproducible evidence of a gain, so the table was not
promoted.

Decision: restore the exact verified 129-block table and rebuild the canonical
artifact. The temporary URL
`http://localhost:8807/?WASM_TPUT=1&WW_ARGS=%2Fv1,%2Fl1&build=safe-after-130-rejection-20260904`
now serves the same artifact hashes as port 8799: JS
`ec9fc0864c8de9f8242b84a84d2f927c7d3b4778ebfd001ae754afc68ee1a1f3`, WASM
`97648ce8f79c17523089b50c65427e9dcaa17a4b8f43406ac1337d6fc8e0940d`, data
`b6e7c288b2cc5f9e5a83a153561d4d385f8eb073e538258ac7ebf65d947e4b63`, index
`455e20ff86b48a6c3e880dd5558bc54c2f749845b2fee6ee7fa343407bd9bcc6`, and
...
# 2026-09-04 19:07 IDT: rejected SSE-interior and 0x005595c0 AOT expansions

Two candidates were tested against the current 173-block control. The first
added 62 interior SSE blocks (`0x00616644` onward), for 235 total; it removed
the SSE addresses from the diagnostic miss list but its normal run reached
only about 93–126 FPS in the warm window. The second added the complete
12-block `0x005595c0` helper path, for 185 total; its normal run reached
about 87–107 FPS. Both candidates rendered real gameplay and accepted Enter
and W, but neither was a measured throughput improvement, so both were
discarded.

Observation: the restored current bundle is again JS
`ec9fc0864c8de9f8242b84a84d2f927c7d3b4778ebfd001ae754afc68ee1a1f3`, WASM
`e79067d0773c427ef0b541e97b87f16c686ff99f863f61401ef63f094e82030c`, data
`b6e7c288b2cc5f9e5a83a153561d4d385f8eb073e538258ac7ebf65d947e4b63`, index
`455e20ff86b48a6c3e880dd5558bc54c2f749845b2fee6ee7fa343407bd9bcc6`, worker
...
audio worklet
`a294aaa599e2505e4069dbdb67de5ace0debeb5ac4ef72a721107ec74f2b1519`.
The exact test URL remains
`http://localhost:8799/?WASM_TPUT=1&WW_ARGS=%2Fv1,%2Fl1&build=fp-173-20260904`.
The Wine tree is dirty only from preserved untracked build/cache/platform
artifacts and the generated AOT table; no sibling checkout was modified.

Decision: keep the 173-block control. Future FPS work should target a
